...
export function DashboardLayout() {
  ...
  const closeMobileSidebar=()=>setIsSidebarOpen(false); const roles=user?.roles ?? []; const permissions=user?.permissions ?? []; const isAdmin=roles.includes("admin"); const isAccountant=roles.includes("accountant"); const isDeliveryOperator=roles.includes("delivery operator"); const isSettlementOperator=roles.includes("settlement operator"); const isSalesVisitor=roles.includes("sales visitor"); const isScientificVisitor=roles.includes("scientific visitor"); const isRestrictedDeliveryOperator=isDeliveryOperator&&!isAdmin&&!isAccountant; const isRestrictedSettlementOperator=isSettlementOperator&&!isAdmin&&!isAccountant; const can=(permission:string)=>isAdmin||permissions.includes(permission); const canSeeNav=(item:NavConfig)=>{ if(isScientificVisitor && !isAdmin && !isAccountant) return ["/visits","/samples","/scientific-inventory"].includes(item.to); if(isRestrictedDeliveryOperator) return ["/deliveries","/order-returns"].includes(item.to); if(isRestrictedSettlementOperator) return ["/customers","/payments"].includes(item.to); if(item.to==="/dashboard") return isAdmin || isAccountant; if(item.adminOnly) return isAdmin; if(item.to==="/catalog") return isAdmin || isAccountant; if(item.to==="/scientific-inventory") return isAdmin || isAccountant || isScientificVisitor; if(item.to==="/products"||item.to==="/inventory") return isAdmin||isAccountant||isSalesVisitor; if(item.to==="/orders") return isAdmin || isAccountant || isSalesVisitor;
  if(item.to==="/invoices") return isAdmin || isAccountant;
  if(item.to==="/expenses") return isAdmin || isAccountant;
  if(item.to==="/deliveries") return isAdmin || isAccountant || isDeliveryOperator || isSettlementOperator;
