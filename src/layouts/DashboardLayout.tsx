...
function NavItem({ label, to, icon, onNavigate }: { label: string; to: string; icon: IconName; onNavigate: () => void }) {
  return <NavLink to={to} onClick={onNavigate} className={({ isActive }) => `group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? "bg-blue-50 text-blue-700 shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>{({ isActive }) => <><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors ${isActive ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500 group-hover:text-slate-700"}`}><Icon name={icon} size={18} /></span><span className="truncate">{label}</span></>}</NavLink>;
}
