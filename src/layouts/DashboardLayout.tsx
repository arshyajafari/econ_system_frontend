...
const navigation: NavConfig[] = [
  ...
  },
  {
    ...
    to: "/doctors",
    icon: "doctors",
    requiredPermission: "doctors.view",
  },
  {
    label: "کارکنان",
    to: "/employees",
  ...
  {
    label: "اطلاعات بانکی کارکنان",
    to: "/employee-banking",
    icon: "payments",
    requiredPermission: "employees.view",
  },
  {
    label: "موقعیت کارکنان",
  ...
  },
  {
    ...
    to: "/samples",
    icon: "samples",
    requiredPermission: "samples.view",
  },
  {
    label: "ارسال‌ها",
    to: "/deliveries",
  ...
  },
  {
    ...
    to: "/inventory",
    icon: "inventory",
    requiredPermission: "inventory_batches.view",
  },
  {
    label: "موجودی ویزیتور علمی",
    to: "/scientific-inventory",
    icon: "inventory",
  },
  {
    label: "مرجوعی‌ها",
    to: "/order-returns",
    icon: "returns",
    requiredPermission: "order_returns.view",
  },
  {
    label: "پرداخت‌ها",
    to: "/payments",
  ...
  const paths: Record<string, ReactNode> = {
    ...
    customers: (
      <>
        ...
        <path d="M3 20c.6-3.4 2.5-5 6-5s5.4 1.6 6 5" />
        <path d="M16 5.2a3 3 0 0 1 0 5.6" />
        <path d="M18 15c1.7.8 2.7 2.3 3 5" />
      </>
    ),
    doctors: (
      <>
    ...
    catalog: (
      <>
        <path d="M4 6.5 12 3l8 3.5L12 10 4 6.5Z" />
    ...
    orders: (
      <>
        ...
        <path d="M3 4h2l2.2 10.2a2 2 0 0 0 2 1.6h8.4a2 2 0 0 0 2-1.5L21 8H6" />
      </>
    ),
    invoices: (
      <>
...
function NavItem({
  ...
  return (
    <NavLink
      ...
      onClick={onNavigate}
      className={({ isActive }) =>
        `group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? "bg-blue-50 text-blue-700 shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`
      }
    >
      {({ isActive }) => (
        <>
          <span
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors ${isActive ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500 group-hover:text-slate-700"}`}
          >
...
export function DashboardLayout() {
  ...
  const isRestrictedDeliveryOperator =
    isDeliveryOperator && !isAdmin && !isAccountant;
  const isRestrictedSettlementOperator =
    isSettlementOperator && !isAdmin && !isAccountant;
  const can = (permission: string) =>
    isAdmin || permissions.includes(permission);
  const canSeeNav = (item: NavConfig) => {
    if (isScientificVisitor && !isAdmin && !isAccountant)
      return ["/visits", "/samples", "/scientific-inventory"].includes(item.to);
    if (isRestrictedDeliveryOperator)
      return ["/deliveries", "/order-returns"].includes(item.to);
    if (isRestrictedSettlementOperator)
      return ["/customers", "/payments"].includes(item.to);
    if (item.to === "/dashboard") return isAdmin || isAccountant;
    if (item.adminOnly) return isAdmin;
    if (item.to === "/catalog") return isAdmin || isAccountant;
    if (item.to === "/scientific-inventory")
      return isAdmin || isAccountant || isScientificVisitor;
    if (item.to === "/products" || item.to === "/inventory")
      return isAdmin || isAccountant || isSalesVisitor;
    if (item.to === "/orders") return isAdmin || isAccountant || isSalesVisitor;
    if (item.to === "/invoices") return isAdmin || isAccountant;
    if (item.to === "/expenses") return isAdmin || isAccountant;
    if (item.to === "/deliveries")
      return (
        isAdmin || isAccountant || isDeliveryOperator || isSettlementOperator
      );
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <aside
        className={`fixed inset-y-0 right-0 z-50 flex w-[272px] flex-col border-l border-slate-200 bg-white shadow-xl shadow-slate-900/5 transition-transform duration-200 lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "translate-x-full"} ${isCollapsed ? "lg:w-[84px]" : ""}`}
      >
        ...
        >
          <div className="space-y-1">
            {navigation.filter(canSeeNav).map((item) => (
              <NavItem
                key={item.to}
                {...item}
            ...
            <NavLink
              ...
              onClick={closeMobileSidebar}
              className={({ isActive }) =>
                `group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium ${isActive ? "bg-blue-50 text-blue-700 shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`
              }
            >
              <span
                className={`relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${unreadCount > 0 ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"}`}
              >
              ...
              <span className="truncate">پیام‌ها و اعلان‌ها</span>
            </NavLink>
          </div>
        </nav>
      ...
      <div
        className={`min-h-screen transition-[padding] duration-200 lg:pr-[272px] ${isCollapsed ? "lg:pr-[84px]" : ""}`}
      >
