...
export function DashboardLayout() {
  ...
  const closeMobileSidebar=()=>setIsSidebarOpen(false); const roles=user?.roles ?? []; const permissions=user?.permissions ?? []; const can=(permission:string)=>roles.includes("admin")||permissions.includes(permission); const canSeeNav=(item:NavConfig)=>item.adminOnly ? roles.includes("admin") : (!item.requiredPermission || can(item.requiredPermission));
