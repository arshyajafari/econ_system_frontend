...
export async function createDelivery(d:DeliveryFormData){const r=await apiClient.post<Delivery>("/deliveries",{...d,recipient_name:d.recipient_name.trim(),recipient_phone:d.recipient_phone.trim(),province:d.province.trim()||undefined,city:d.city.trim()||undefined,address:d.address.trim(),description:d.description.trim()||undefined});return r.data}
export async function deleteDelivery(id:string){await apiClient.delete(`/deliveries/${id}`)}
export async function updateDelivery(id:string,d:Omit<DeliveryFormData,"order_id">){const r=await apiClient.put<Delivery>(`/deliveries/${id}`,{...d,recipient_name:d.recipient_name.trim(),recipient_phone:d.recipient_phone.trim(),province:d.province.trim()||undefined,city:d.city.trim()||undefined,address:d.address.trim(),description:d.description.trim()||undefined});return r.data}
...
export async function getAvailableOrders(): Promise<DeliveryOrderSource[]> {
  const r = await apiClient.get<{ data: DeliveryOrderSource[] } | DeliveryOrderSource[]>("/deliveries/available-orders");
