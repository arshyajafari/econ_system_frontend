...
export function EmployeeLocationsPage() {
  ...
  return <section className="space-y-6 p-4 md:p-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-bold text-gray-900">موقعیت کارکنان</h1><p className="mt-1 text-sm text-gray-500">آخرین موقعیت ثبت‌شده هر کارمند فعال.</p></div><button type="button" onClick={() => window.location.reload()} disabled={loading} className="rounded-lg border bg-white px-4 py-2.5 text-sm disabled:opacity-60">{loading ? "در حال بروزرسانی..." : "بروزرسانی"}</button></div>
