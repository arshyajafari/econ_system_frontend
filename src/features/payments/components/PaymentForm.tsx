...
export function PaymentForm({ payment, invoices, isSubmitting, error, onSubmit, onCancel }: PaymentFormProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!invoiceId) { setInvoicePayments([]); setBalanceError(null); setIsLoadingBalance(false); return () => { cancelled = true; }; }
    async function loadInvoicePayments() {
      setIsLoadingBalance(true); setBalanceError(null);
      try { const response = await getPayments({ invoice_id: invoiceId, per_page: 100 }); if (!cancelled) setInvoicePayments(response.data); }
      catch (requestError: unknown) { if (!cancelled) setBalanceError(requestError instanceof ApiError && requestError.message ? requestError.message : "خطا در دریافت وضعیت پرداخت فاکتور."); }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!invoiceId) return;
    const numericAmount = Number(amount); if (!Number.isFinite(numericAmount) || numericAmount <= 0) return;
  ...
  return <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-5">
    <div className="mb-5"><h2 className="text-lg font-semibold text-gray-900">{payment ? "ویرایش پرداخت" : "پرداخت جدید"}</h2><p className="mt-1 text-sm text-gray-500">فقط فاکتورهای صادرشده قابل پرداخت هستند.</p></div>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <div><label className="mb-1.5 block text-sm font-medium text-gray-700">فاکتور</label><select required disabled={isSubmitting || Boolean(payment)} value={invoiceId} onChange={(event) => setInvoiceId(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm disabled:bg-gray-100"><option value="">انتخاب فاکتور</option>{invoices.map((invoice) => <option key={invoice.id} value={invoice.id}>{invoice.code} — {numberFormatter.format(Number(invoice.total_amount || 0))}</option>)}</select></div>
      ...
      <div><label className="mb-1.5 block text-sm font-medium text-gray-700">مبلغ</label><input required min="0.01" step="any" type="number" disabled={isSubmitting} value={amount} onChange={(event) => setAmount(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />{selectedInvoice ? <div className="mt-2 text-xs text-gray-500">{isLoadingBalance ? "در حال محاسبه مانده..." : `مانده قابل پرداخت: ${numberFormatter.format(remainingAmount)}`}</div> : null}</div>
