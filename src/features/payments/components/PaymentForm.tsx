...
export function PaymentForm({ payment, invoices, isSubmitting, error, onSubmit, onCancel }: PaymentFormProps) {
  ...
  const invoiceRemainingBeforePending = selectedInvoice?.remaining_amount != null
    ...
    : selectedInvoice
      ? Math.max(0, Number(selectedInvoice.total_amount || 0) - confirmedAmount)
      : 0;
  ...
  return <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-5">
    <div className="mb-5"><h2 className="text-lg font-semibold text-gray-900">{payment ? "ویرایش پرداخت" : "پرداخت جدید"}</h2><p className="mt-1 text-sm text-gray-500">فقط فاکتورهای صادرشده، ارسال‌شده و تسویه‌نشده قابل پرداخت هستند.</p></div>
  ...
  return (
    <option key={invoice.id} value={invoice.id}>
      {invoice.code} — مانده: {numberFormatter.format(remaining)}
    </option>
  );
})}</select>{!payment && customerId ? <p className="mt-1.5 text-xs text-gray-500">فقط فاکتورهای صادرشده، ارسال‌شده و دارای مانده این مشتری نمایش داده می‌شوند؛ فاکتور تسویه‌شده یا ارسال‌نشده در این فهرست نمی‌آید.</p> : null}</div>
      ...
      <div><label className="mb-1.5 block text-sm font-medium text-gray-700">مبلغ</label><FormattedNumberInput required min="0.01" step="0.01" dir="ltr" disabled={isSubmitting} value={amount} onValueChange={setAmount} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-right" placeholder="مثلاً 1,500,000" />{customerId ? <div className="mt-2 text-xs text-gray-500">{isLoadingCustomerBalance ? "در حال دریافت مانده حساب مشتری..." : customerBalance === null ? "مانده حساب مشتری: —" : customerCredit > 0 ? `اعتبار مشتری: ${numberFormatter.format(customerCredit)}` : `مانده حساب مشتری: ${numberFormatter.format(customerBalance)}`}</div> : null}{selectedInvoice ? <div className="mt-1 text-xs text-gray-500">{isLoadingBalance ? "در حال محاسبه مانده فاکتور..." : `مانده فاکتور: ${numberFormatter.format(remainingAmount)}`}</div> : null}{selectedInvoice && Number(amount || 0) > remainingAmount ? <div className="mt-1 text-xs font-medium text-blue-600">مازاد پرداخت: {numberFormatter.format(Math.max(0, Number(amount || 0) - remainingAmount))} — به‌عنوان اعتبار مشتری ثبت می‌شود.</div> : null}</div>
