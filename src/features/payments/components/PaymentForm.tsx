...
export function PaymentForm({ payment, invoices, isSubmitting, error, onSubmit, onCancel }: PaymentFormProps) {
  ...
  return <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <div><label className="mb-1.5 block text-sm font-medium text-gray-700">فاکتور</label><select required disabled={isSubmitting || Boolean(payment)} value={invoiceId} onChange={(event) => handleInvoiceChange(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm disabled:bg-gray-100"><option value="">انتخاب فاکتور</option>{invoices.map((invoice) => <option key={invoice.id} value={invoice.id}>{invoice.code} — {numberFormatter.format(Number(invoice.total_amount || 0))}</option>)}</select></div>
