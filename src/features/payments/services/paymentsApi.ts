...
export async function createPayment(payload: PaymentFormData): Promise<Payment> {
  const response = await apiClient.post<Payment>("/payments", toMultipart(payload));
  return response.data;
}

export async function updatePayment(id: string, payload: PaymentFormData): Promise<Payment> {
  if (!payload.receipt_image) {
    const response = await apiClient.put<Payment>(`/payments/${id}`, normalizePaymentPayload(payload, true));
    return response.data;
  }

  const form = toMultipart(payload, true);
  const response = await apiClient.post<Payment>(`/payments/${id}`, form);
  return response.data;
}
...
export async function performPaymentStatusAction(id: string, action: PaymentStatusAction): Promise<Payment> {
  switch (action) {
    case "confirm": return confirmPayment(id);
    case "cancel": return cancelPayment(id);
  }
}
