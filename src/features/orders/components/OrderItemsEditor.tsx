...
const emptyItem = (): OrderItemFormData => ({
  ...
  offer_type:"none", offer_buy_quantity:"0", offer_free_quantity:"0", offer_title:"",
});
  ...
  return <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
    <div className="border-b border-gray-100 bg-gradient-to-l from-gray-50 to-white px-5 py-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gray-900 text-sm font-bold text-white">01</span><div><h3 className="font-bold text-gray-900">اقلام سفارش</h3><p className="mt-0.5 text-xs text-gray-500">محصول، تعداد، قیمت و در صورت نیاز درصد تخفیف یا تعداد هدیه را ثبت کنید.</p></div></div></div>
      ...
      {items.map((item,index)=>{
        ...
        return <article key={index} className="overflow-hidden rounded-2xl border border-gray-200 bg-gray-50/60">
          ...
          <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2 lg:grid-cols-6">
            ...
            <Field label="تعداد خرید" required><input dir="ltr" type="number" min={1} step={1} value={item.quantity} onChange={e=>{const quantity=Math.max(1,Math.trunc(Number(e.target.value)||1));updateItem(index,{quantity,offer_buy_quantity:getFree(item)>0?String(quantity):"0"})}} disabled={disabled} className={inputClass}/></Field>
            ...
            <Field label="تعداد هدیه"><input aria-label="تعداد هدیه" dir="ltr" type="number" min={0} step={1} value={getFree(item) || ""} onChange={e=>updateGift(index,e.target.value)} disabled={disabled} className={inputClass} placeholder="اختیاری"/></Field>
          ...
          {free>0 ? <div className="mx-4 mb-4 rounded-xl border border-emerald-100 bg-emerald-50/40 px-3 py-2.5">
            <div className="text-xs text-emerald-700">این ردیف: <strong>{numberFormatter.format(item.quantity)}</strong> عدد خرید + <strong>{numberFormatter.format(free)}</strong> عدد هدیه → تحویل <strong>{numberFormatter.format(deliveryQuantity)}</strong> عدد</div>
          </div> : null}
