...
function getItemDiscountAmount(form: OrderFormData): number {
...
export function OrderForm({ order, customers, products, isSubmitting, error, onSubmit, onCancel }: OrderFormProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (form.discount_type === "fixed" && (!Number.isFinite(discountValue) || discountValue < 0 || discountValue > (getItemsSubtotal() - getItemDiscountAmount(form)))) return;
  ...
  const itemDiscountAmount = getItemDiscountAmount(form);
