...
function orderToForm(order: Order): OrderFormData {
  return {
    ...
    items: order.items.map((item) => ({
      ...
      offer_type: item.free_quantity > 0 ? "buy_x_get_y" : "none",
      offer_buy_quantity: item.free_quantity > 0 ? String(item.quantity) : "0",
      offer_free_quantity: String(Math.max(0, item.free_quantity ?? 0)),
...
function getDiscountAmount(form: OrderFormData): number {
  ...
  if (form.discount_type === "percentage") {
    return Math.min(
      subtotal,
      Math.round(subtotal * ((Number(form.discount_value) || 0) / 100) * 100) /
        100,
    );
  }
...
export function OrderForm({
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      (!Number.isFinite(discountValue) ||
        discountValue < 0 ||
        discountValue > getItemsSubtotal() - itemDiscountAmount)
    )
      return;
    if (
      form.items.some((item) => {
        ...
        if (
          ...
          return true;
        const gift = Number(item.offer_free_quantity);
        if (!Number.isFinite(gift) || gift < 0 || !Number.isInteger(gift))
          return true;
        return false;
      })
    )
  ...
  const finalAmount = Math.max(
    0,
    subtotal - itemDiscountAmount - discountAmount,
  );
  const isValid =
    ...
    form.items.every((item) => {
      ...
      const offerValid =
        Number.isFinite(Number(item.offer_free_quantity)) &&
        Number(item.offer_free_quantity) >= 0 &&
        Number.isInteger(Number(item.offer_free_quantity));
      const discountValid =
        item.discount_type === "none" ||
        (Number.isFinite(itemValue) &&
          itemValue >= 0 &&
          (item.discount_type !== "percentage" || itemValue <= 100) &&
          (item.discount_type !== "fixed" ||
            itemValue <= Number(item.quantity) * Number(item.unit_price)));
      return (
      ...
      (Number.isFinite(Number(form.discount_value)) &&
        ...
        (form.discount_type !== "fixed" ||
          Number(form.discount_value) <=
            Math.max(0, subtotal - itemDiscountAmount))));
        ...
        <section className="rounded-xl border border-gray-200 bg-gray-50 p-4">
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs text-gray-500">
              تخفیف سفارش روی مبلغ نهایی فاکتور اعمال می‌شود.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Field label="نوع تخفیف">
              <select
                ...
                onChange={(event) =>
                  update(
                    "discount_type",
                    event.target.value as OrderDiscountType,
                  )
                }
                disabled={isSubmitting}
                className={inputClass}
            ...
            >
              <FormattedNumberInput
                ...
                onValueChange={(value) => update("discount_value", value)}
                disabled={isSubmitting || form.discount_type === "none"}
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            <Field label="عنوان آفر">
              <input
                ...
                onChange={(event) => update("offer_title", event.target.value)}
                disabled={isSubmitting}
                className={inputClass}
                placeholder="مثلاً آفر ویژه داروخانه"
              />
            </Field>
            <Field label="توضیحات آفر">
              <textarea
                ...
                onChange={(event) =>
                  update("offer_description", event.target.value)
                }
                disabled={isSubmitting}
                rows={1}
                className={inputClass}
                placeholder="شرایط یا توضیح آفر..."
              />
            </Field>
          </div>
...
function Summary({
  ...
  return (
    <div
      className={`rounded-lg border border-gray-200 bg-white px-4 py-3 ${emphasized ? "ring-1 ring-gray-300" : ""}`}
    >
      <div className="text-xs text-gray-500">{label}</div>
      <div
        dir="ltr"
        className={`mt-1 font-bold ${emphasized ? "text-lg text-gray-900" : "text-sm text-gray-800"}`}
      >
