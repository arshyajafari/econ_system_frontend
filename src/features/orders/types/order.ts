...
export type OrderProduct = { id: string; code: string; title: string; sale_price?: string | null };
...
export type Order = { id: string; code: string; customer: OrderCustomer | null; sales_employee: OrderSalesEmployee | null; status: OrderStatus; ordered_at: string | null; description: string | null; meta: Record<string, unknown> | null; items: OrderItem[]; created_at: string | null; updated_at: string | null; deleted_at: string | null };
...
export type OrderProductOption = { id: string; code: string; title: string; sale_price?: string | null };
export type OrderListParams = { search?: string; status?: OrderStatus; customer_id?: string; sales_employee_id?: string; ordered_from?: string; ordered_to?: string; sort?: string; page?: number; per_page?: number };
...
export type OrderFormData = { customer_id: string; sales_employee_id: string; description: string; items: OrderItemFormData[] };
export type OrderStatusOption = { value: OrderStatus; label: string };
export const ORDER_STATUS_OPTIONS: OrderStatusOption[] = [{ value: "draft", label: "پیش‌نویس" }, { value: "pending", label: "در انتظار تأیید" }, { value: "confirmed", label: "تأیید شده" }, { value: "cancelled", label: "لغو شده" }, { value: "completed", label: "تکمیل شده" }];
export function getOrderStatusLabel(status: OrderStatus): string { return ORDER_STATUS_OPTIONS.find((option) => option.value === status)?.label ?? status; }
