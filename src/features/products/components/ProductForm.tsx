...
function productToForm(product: Product): ProductFormData {
  return { brand_id: product.brand?.id ?? "", product_category_id: product.category?.id ?? "", title: product.title, barcode: product.barcode ?? "", sort_order: product.sort_order, status: product.status, image: product.image ?? "", description: product.description ?? "" };
}
...
export function ProductForm({ product, brands, categories, isSubmitting, error, onSubmit, onCancel }: ProductFormProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); if (!form.title.trim() || !form.brand_id || !form.product_category_id) return; onSubmit(form, imageFile); }
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Field label="تصویر محصول"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setImageFile(event.target.files?.[0] ?? null)} disabled={isSubmitting} className={`${inputClass} file:mr-3 file:rounded-md file:border-0 file:bg-gray-100 file:px-3 file:py-1.5`} />{imageFile ? <p className="mt-1 text-xs text-gray-500">فایل انتخاب‌شده: {imageFile.name}</p> : form.image ? <p className="mt-1 text-xs text-gray-500">تصویر فعلی حفظ می‌شود مگر فایل جدید انتخاب کنید.</p> : null}</Field>
        </div>
