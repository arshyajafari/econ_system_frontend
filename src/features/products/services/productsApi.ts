...
type ProductApiPayload = { brand_id: string; product_category_id: string; title: string; sale_price: string; barcode?: string; sort_order: number; status: ProductStatus; image?: string; description?: string };
...
function normalizeProductPayload(payload: ProductFormData): ProductApiPayload { return { brand_id: payload.brand_id, product_category_id: payload.product_category_id, title: payload.title.trim(), sale_price: payload.sale_price, barcode: payload.barcode.trim() || undefined, sort_order: Math.max(0, payload.sort_order), status: payload.status, image: payload.image.trim() || undefined, description: payload.description.trim() || undefined }; }
