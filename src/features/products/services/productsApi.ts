...
import type { Product, ProductBrand, ProductCategory, ProductFormData, ProductListParams, ProductListResponse, ProductStatus } from "../types/product";

type ProductApiPayload = { brand_id: string; product_category_id: string; title: string; barcode?: string; sort_order: number; status: ProductStatus; image?: string; description?: string };
...
export async function createProduct(payload: ProductFormData): Promise<Product> {
  const response = await apiClient.post<Product>("/products", normalizeProductPayload(payload));
  const withPrice = await setProductPrice(response.data.id, payload.sale_price);
  return payload.image_file ? uploadProductImage(response.data.id, payload.image_file) : { ...response.data, current_price: withPrice };
}
export async function updateProduct(id: string, payload: ProductFormData): Promise<Product> {
  const response = await apiClient.put<Product>(`/products/${id}`, normalizeProductPayload(payload));
  const withPrice = await setProductPrice(id, payload.sale_price);
  const product = payload.image_file ? await uploadProductImage(id, payload.image_file) : response.data;
  return { ...product, current_price: withPrice };
}
export async function setProductPrice(id: string, sale_price: string): Promise<Product["current_price"]> {
  const response = await apiClient.post<Product["current_price"]>(`/products/${id}/price`, { sale_price });
...
function normalizeProductPayload(payload: ProductFormData): ProductApiPayload { return { brand_id: payload.brand_id, product_category_id: payload.product_category_id, title: payload.title.trim(), barcode: payload.barcode.trim() || undefined, sort_order: Math.max(0, payload.sort_order), status: payload.status, image: payload.image.trim() || undefined, description: payload.description.trim() || undefined }; }
