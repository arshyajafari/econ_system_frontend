...
type Props = {
  ...
  isLoading?: boolean;
  onUpdated?: (item: OrderReturn) => void;
};
...
function getReturnTotal(orderReturn: OrderReturn): number {
  return Number(orderReturn.return_amount || 0);
}

export function OrderReturnTable({
  orderReturns,
  isLoading = false,
  onUpdated,
}: Props) {
        ...
        <table className="min-w-[1050px] w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {orderReturns.map((orderReturn) => (
              <tr key={orderReturn.id} className="hover:bg-gray-50">
                <td
                ...
                <td className="px-5 py-4">
                  <span
                    className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${getStatusClass(orderReturn.status)}`}
                  >
                    {getStatusLabel(orderReturn.status)}
                  </span>
                </td>
                <td className="px-5 py-4">
                  {orderReturn.delivered_at ? (
                    <div className="text-xs font-medium text-green-700">
                      تحویل گرفته شد
                      <div className="mt-1 text-gray-500">
                        {formatJalaliDateTime(orderReturn.delivered_at)}
                      </div>
                    </div>
                  ) : (
                    <span className="text-xs text-gray-400">تحویل نشده</span>
                  )}
                </td>
                <td className="px-5 py-4 text-gray-500">
                  {formatJalaliDateTime(orderReturn.created_at)}
                </td>
                <td className="px-5 py-4">
                    ...
                    orderReturn.status !== "cancelled" ? (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const updated = await receiveOrderReturn(
                              orderReturn.id,
                            );
                            onUpdated?.(updated);
                          } catch (e: unknown) {
