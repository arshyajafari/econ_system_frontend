...
export function OrderReturnFormPage() {
    ...
    async function load() {
      ...
      try {
        if (returnId) {
          ...
          setReturnAmount(
            response.is_manual_amount ? String(response.return_amount) : "",
          );
          setItems(
          ...
          if (response.order?.id) {
            const loadedOrder = await getReturnableOrder(response.order.id);
            if (!cancelled) {
              setOrder(loadedOrder);
              setSelectedOrderId(loadedOrder.id);
            }
          ...
          const loadedOrder = await getReturnableOrder(orderId);
          if (!cancelled) {
            setOrder(loadedOrder);
            setSelectedOrderId(loadedOrder.id);
            if (loadedOrder.status !== "completed")
        ...
        if (!cancelled)
          setError(
            error instanceof ApiError && error.message
              ? error.message
              : "اطلاعات مرجوعی قابل دریافت نیست. لطفاً وضعیت سفارش را بررسی کنید.",
          );
      } finally {
  ...
  function toggleItem(orderItemId: string) {
    setItems((current) =>
      current.some((item) => item.orderItemId === orderItemId)
        ? current.filter((item) => item.orderItemId !== orderItemId)
        : [
            ...current,
            { orderItemId, quantity: 1, freeQuantity: 0, description: "" },
          ],
    );
  }
  ...
  async function handleSubmit() {
    if (isSaving) return;
    if (!order) {
      setError("ابتدا یک سفارش قابل مرجوعی را انتخاب کنید.");
      return;
    }
    if (!items.length) {
    ...
    const normalizedReturnAmount = returnAmount.trim();
    if (normalizedReturnAmount) {
      const numericReturnAmount = Number(normalizedReturnAmount);
      if (!Number.isFinite(numericReturnAmount) || numericReturnAmount <= 0) {
        setError("مبلغ دستی مرجوعی باید بیشتر از صفر باشد.");
        return;
      }
    }
    const payload: OrderReturnFormData = {
      ...
      ...(normalizedReturnAmount
        ? { return_amount: normalizedReturnAmount }
        : {}),
      items: items.map((item) => ({
        ...
        free_quantity: Math.max(
          0,
          Math.min(
            Math.trunc(Number(item.freeQuantity) || 0),
            Math.trunc(Number(item.quantity) || 0),
          ),
        ),
  ...
  return (
    <section className="space-y-6 p-4 md:p-6">
      <div>
        <BackButton fallbackTo="/order-returns" label="بازگشت به مرجوعی‌ها" />
        ...
        <>
              ...
              <table className="min-w-[900px] w-full text-sm">
                <thead className="bg-gray-50 text-right text-gray-600">
                  <tr>
                    ...
                    <th className="px-5 py-3 font-medium">تعداد خرید</th>
                    <th className="px-5 py-3 font-medium">رایگان</th>
                    ...
                    return (
                      <tr key={orderItem.id}>
                        ...
                        <td className="px-5 py-4">
                          ...
                          {Number(orderItem.free_quantity || 0) > 0 ? (
                            <div className="mt-1 text-xs text-emerald-600">
                              + {orderItem.free_quantity} رایگان
                            </div>
                          ) : null}
                        </td>
                        <td className="px-5 py-4">
                          {selected ? (
                            <input
                              type="number"
                              min={0}
                              max={Number(orderItem.free_quantity || 0)}
                              value={selected.freeQuantity}
                              onChange={(event) =>
                                updateItem(orderItem.id, {
                                  freeQuantity: Number(event.target.value),
                                })
                              }
                              disabled={isSaving}
                              className="w-24 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2"
                            />
                          ) : (
                            "—"
                          )}
                        ...
                        <td className="px-5 py-4">
                          {selected ? (
                            <input
                              ...
                              min={1}
                              max={Number(
                                orderItem.fulfillment_quantity ??
                                  Number(orderItem.quantity) +
                                    Number(orderItem.free_quantity || 0),
                              )}
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-5">
            ...
            <FormattedNumberInput
              ...
              disabled={isSaving}
              className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-right"
              placeholder="خالی = محاسبه خودکار"
            />
            <p className="mt-2 text-xs leading-6 text-gray-600">
              اگر مبلغ را وارد کنید، همین مبلغ مبنای اعتبار مشتری خواهد بود؛
              برای حالتی که مشتری با قیمت فعلی محصول مرجوع می‌کند. اگر خالی
              بماند، مبلغ طبق مبلغ فاکتور محاسبه می‌شود.
            </p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            <label
