...
export function SampleForm({
  ...
  const [visitId, setVisitId] = useState(sample?.visit?.id ?? ""),
    [productId, setProductId] = useState(sample?.product?.id ?? ""),
    [quantity, setQuantity] = useState(String(sample?.quantity ?? 1)),
    [description, setDescription] = useState(sample?.description ?? "");
    ...
    >
      <div className="flex items-center justify-between">
        <h2 className="font-semibold">
          {sample ? "ویرایش نمونه" : "ثبت نمونه جدید"}
        </h2>
        <button
          type="button"
          onClick={onCancel}
      ...
      {!sample && (
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-1 text-sm">
            ...
            >
              <option value="">انتخاب بازدید</option>
              {visits.map((visit) => (
                <option key={visit.id} value={visit.id}>
                  {visit.doctor?.name ?? "پزشک نامشخص"} —{" "}
                  {formatJalaliDate(visit.visit_date)}
                </option>
              ))}
            </select>
          </label>
      ...
      {selectedProduct?.available_quantity !== undefined && !sample ? (
        <p className="text-xs text-gray-500">
          موجودی قابل استفاده برای نمونه:{" "}
