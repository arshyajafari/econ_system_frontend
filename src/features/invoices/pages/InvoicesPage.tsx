    ...
    <section className="space-y-6 p-4 md:p-6">
      ...
      {error ? (
        ...
        >
          ...
          <button
            type="button"
            onClick={() => void load()}
            className="rounded border border-red-200 px-3 py-1.5"
          >
            تلاش مجدد
          </button>
        </div>
      ...
      <div className="grid gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <select
          value={settled}
          onChange={(e) => {
            setSettled(e.target.value as "" | "true" | "false");
            setPage(1);
          }}
          className="rounded-lg border border-gray-300 px-3 py-2 text-sm"
        >
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
        <table className="min-w-[1000px] w-full text-sm">
          <thead className="bg-gray-50 text-right text-gray-600">
            <tr>
              <th className="px-5 py-3 font-medium">کد فاکتور</th>
              <th className="px-5 py-3 font-medium">مشتری</th>
              <th className="px-5 py-3 font-medium">سفارش</th>
              <th className="px-5 py-3 font-medium">وضعیت</th>
              <th className="px-5 py-3 font-medium">تسویه</th>
              <th className="px-5 py-3 font-medium">مبلغ نهایی</th>
              <th className="px-5 py-3 font-medium">سررسید</th>
              <th />
            </tr>
          </thead>
            ...
            {!loading
              ? invoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4">{invoice.order?.code ?? "—"}</td>
                    <td className="px-5 py-4">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(invoice.status)}`}
                      >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-medium ${invoice.is_settled ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                      >
                    ...
                    <td className="px-5 py-4">
                      {formatJalaliDate(invoice.due_date)}
                    </td>
