    ...
    try {
      ...
      setError(
        e instanceof ApiError && e.message
          ? e.message
          : "خطا در دریافت سفارش‌های تکمیل‌شده.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="space-y-6 p-4 md:p-6">
      <div>
        <div className="flex justify-end">
          <button
            ...
            className="mb-3 text-left text-sm font-medium text-gray-500 hover:text-gray-900"
          >
            بازگشت ←
          </button>
        </div>
        <h1 className="text-2xl font-bold text-gray-900">ثبت فاکتور</h1>
        <p className="mt-1 text-sm text-gray-500">
          فاکتور از روی یک سفارش تکمیل‌شده ساخته می‌شود.
        </p>
      </div>
      {error ? (
        <div
      ...
      <div className="max-w-2xl rounded-xl border border-gray-200 bg-white p-6">
        {loading ? (
          <div className="text-sm text-gray-500">در حال دریافت سفارش‌ها...</div>
        ) : (
          <>
            ...
            >
              ...
              {orders.map((order) => (
                <option key={order.id} value={order.id}>
                  {order.code} — {order.customer?.customer_name ?? "بدون مشتری"}
                </option>
              ))}
            </select>
            {orders.length === 0 ? (
              <p className="mt-3 text-sm text-gray-500">
                سفارش تکمیل‌شده‌ای برای صدور فاکتور پیدا نشد.
              </p>
            ) : null}
            <div className="mt-5 flex gap-2">
              ...
              >
                {busy ? "در حال ایجاد..." : "ایجاد فاکتور"}
              </button>
              <button
                type="button"
                onClick={() => navigate("/invoices")}
