...
export function InvoiceDetailsPage() {
  ...
  const canCreateReturn =
    user?.roles.some((role) =>
      ["admin", "accountant", "sales visitor", "delivery operator"].includes(
        role,
      ),
    ) ?? false;
  ...
  async function action(kind: "issue" | "cancel") {
    ...
    if (
      kind === "cancel" &&
      !window.confirm(`آیا از لغو فاکتور «${invoice.code}» مطمئن هستید؟`)
    )
      return;
    setBusy(true);
    setError(null);
  ...
  if (!invoice)
    return (
      <section className="space-y-4 p-4 md:p-6">
        <BackButton fallbackTo="/invoices" label="بازگشت به فاکتورها" />
        <div
          role="alert"
  ...
  const remaining = Math.max(
    0,
    Number(invoice.total_amount) - confirmed - pending,
  );
  return (
    <section className="space-y-6 p-4 md:p-6">
          ...
          {invoice.status === "draft" ? (
            <>
              ...
              {isAdmin ? (
                ...
                <span className="rounded-lg bg-yellow-50 px-4 py-2 text-sm text-yellow-700">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border bg-white p-5">
          <div className="text-sm text-gray-500">سفارش</div>
          <div className="mt-2 font-semibold">{invoice.order?.code ?? "—"}</div>
          {invoice.order?.status && invoice.order.status !== "completed" ? (
            <div className="mt-1 text-xs text-amber-600">
              ثبت مرجوعی پس از تکمیل سفارش فعال می‌شود.
            </div>
          ) : null}
        </div>
        <div className="rounded-xl border bg-white p-5">
          <div className="text-sm text-gray-500">مانده</div>
          <div dir="ltr" className="mt-2 font-bold">
            {money(remaining)}
          </div>
        </div>
      </div>
      <div className="overflow-x-auto rounded-xl border bg-white">
        <table className="min-w-[750px] w-full text-sm">
          <thead className="bg-gray-50 text-right text-gray-600">
            <tr>
              <th className="px-5 py-3">محصول</th>
              <th className="px-5 py-3">تعداد خرید</th>
              <th className="px-5 py-3">رایگان</th>
              <th className="px-5 py-3">آفر</th>
              <th className="px-5 py-3">قیمت واحد</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {invoice.items.map((item) => (
              <tr key={item.id}>
                <td className="px-5 py-4">{item.product?.title ?? "—"}</td>
                <td className="px-5 py-4">{nf.format(item.quantity)}</td>
                <td className="px-5 py-4">
                  <span className="font-semibold text-emerald-700">
                    {nf.format(Number(item.free_quantity || 0))}
                  </span>
                </td>
                <td className="px-5 py-4">
                  {item.offer_type === "buy_x_get_y" ? (
                    <div>
                      <div className="font-semibold text-emerald-700">
                        {nf.format(Number(item.offer_buy_quantity || 0))} +{" "}
                        {nf.format(
                          Number(item.offer_free_quantity_per_cycle || 0),
                        )}
                      </div>
                      {item.offer_title ? (
                        <div className="mt-1 text-xs text-gray-500">
          ...
          <tfoot>
            <tr className="border-t">
              ...
              <td dir="ltr" className="px-5 py-4 font-bold">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border bg-white p-5">
          <h2 className="font-semibold">خلاصه مبلغ</h2>
          <div className="mt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span>تخفیف</span>
              <b>{money(invoice.discount_amount)}</b>
            </div>
            <div className="flex justify-between">
              <span>مالیات</span>
              <b>{money(invoice.tax_amount)}</b>
            </div>
        ...
        <div className="rounded-xl border bg-white p-5">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <div>
              تأییدشده: <b>{money(confirmed)}</b>
            </div>
            <div>
              در انتظار: <b>{money(pending)}</b>
            </div>
            <div>
              مانده: <b>{money(remaining)}</b>
            </div>
          </div>
        </div>
      </div>
