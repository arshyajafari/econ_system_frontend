...
export function EmployeeForm({ employee, isSubmitting, error, onSubmit, onCancel }: Props) {
  ...
  return <section className="rounded-xl border border-gray-200 bg-white p-5">
    ...
    <form onSubmit={submit} className="space-y-6">
      ...
      <div className="border-t border-gray-100 pt-6"><div className="mb-4"><h3 className="text-sm font-semibold">حساب ورود و دسترسی</h3><p className="mt-1 text-xs text-gray-500">نقش‌ها از فعالیت شغلی جدا هستند و می‌توان چند نقش را هم‌زمان اختصاص داد.</p></div><div className="grid grid-cols-1 gap-4 md:grid-cols-3"><Field label="نام کاربری" required={!employee}><input dir="rtl" autoComplete="username" value={form.login} onChange={(e) => update("login", e.target.value)} disabled={isSubmitting} placeholder={employee ? "برای تغییر وارد کنید" : "مثلاً ahmad"} className={input} /></Field><Field label={employee ? "رمز عبور جدید" : "رمز عبور"} required={!employee}><input dir="rtl" type="password" autoComplete={employee ? "new-password" : "new-password"} value={form.password} onChange={(e) => update("password", e.target.value)} disabled={isSubmitting} className={input} /></Field><Field label="تکرار رمز عبور" required={!employee}><input dir="rtl" type="password" autoComplete="new-password" value={form.password_confirmation} onChange={(e) => update("password_confirmation", e.target.value)} disabled={isSubmitting} className={input} /></Field></div><div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3">{EMPLOYEE_ROLE_OPTIONS.map((o) => <label key={o.value} className="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm hover:bg-gray-50"><input type="checkbox" checked={form.roles.includes(o.value)} onChange={() => toggle<EmployeeRole>("roles", o.value)} disabled={isSubmitting} />{o.label}</label>)}</div></div>

      <div className="border-t border-gray-100 pt-6"><h3 className="mb-4 text-sm font-semibold">آدرس</h3><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><Field label="استان"><input value={form.address.province} onChange={(e) => updateAddress("province", e.target.value)} disabled={isSubmitting} className={input} /></Field><Field label="شهر"><input value={form.address.city} onChange={(e) => updateAddress("city", e.target.value)} disabled={isSubmitting} className={input} /></Field><Field label="کد پستی"><input dir="ltr" value={form.address.postal_code} onChange={(e) => updateAddress("postal_code", e.target.value)} disabled={isSubmitting} className={input} /></Field><Field label="آدرس"><textarea rows={1} value={form.address.address} onChange={(e) => updateAddress("address", e.target.value)} disabled={isSubmitting} className={input} /></Field></div></div>
