...
export function ReportsPage() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => void loadReport(monthStart, today), 0);
  ...
  return (
    <section className="space-y-6 p-4 md:p-6">
      <header>
        ...
        <p className="text-sm text-gray-500">گزارش فروش، پرداخت، سفارش و مرجوعی در بازه انتخابی؛ پیش‌فرض از ابتدای ماه</p>
      </header>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (from <= to) void loadReport(from, to); else setError("تاریخ شروع باید قبل از تاریخ پایان باشد."); }} className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-3">
        <label className="text-sm">از تاریخ<JalaliDateInput value={from} onChange={setFrom} className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
        <label className="text-sm">تا تاریخ<JalaliDateInput value={to} onChange={setTo} className="mt-1 w-full rounded-lg border px-3 py-2" /></label>
        <button disabled={loading || !from || !to} className="self-end rounded-lg bg-gray-900 px-4 py-2 text-sm text-white disabled:opacity-50">{loading ? "در حال دریافت..." : "نمایش گزارش"}</button>
      </form>

      {error ? <div role="alert" className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</div> : null}

      {loading && !report ? (
        <div className="rounded-xl border border-dashed p-10 text-center text-sm text-gray-500">در حال دریافت گزارش...</div>
      ) : report ? (
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <ReportCard label="مانده حساب مشتریان" value={money(report.receivables.total)} unit="تومان · مانده جاری" />
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <div className="rounded-xl border bg-white p-4">
              ...
              <div className="mb-4 rounded-lg bg-green-50 px-3 py-2 text-sm text-green-800">
                {money(report.payments.count)} پرداخت به مبلغ {money(report.payments.total)} تومان وصول و تأیید شده است؛ {money(report.payments.pending_count)} پرداخت به مبلغ {money(report.payments.pending_total)} تومان هنوز در انتظار تأیید است. پرداخت‌های لغوشده در این گزارش محاسبه نمی‌شوند.
              </div>
            ...
            <div className="rounded-xl border bg-white p-4">
              ...
              {report.top_products.length === 0 ? <p className="text-sm text-gray-500">داده‌ای وجود ندارد.</p> : (
                <div className="divide-y">
                  {report.top_products.map((product, index) => (
                    <div key={product.id} className="flex items-center justify-between gap-3 py-3 text-sm">
                      <div><span className="ml-2 text-gray-400">{index + 1}</span><span className="font-medium">{product.title}</span><span className="mr-2 text-xs text-gray-400">{product.code}</span></div>
                      <div className="text-left"><div>{money(product.total_amount)} تومان</div><div className="text-xs text-gray-400">{money(product.quantity)} عدد</div></div>
                    </div>
                  ))}
                </div>
              )}
