...
function ReportCard({ label, value, unit }: { label: string; value: string; unit: string }) {
  return <div className="rounded-xl border bg-white p-4"><div className="text-xs text-gray-500">{label}</div><div className="mt-2 text-xl font-bold">{value}</div><div className="mt-1 text-xs text-gray-400">{unit}</div></div>;
}

function getLocalDateString(date = new Date()): string { const year = date.getFullYear(); const month = String(date.getMonth() + 1).padStart(2, "0"); const day = String(date.getDate()).padStart(2, "0"); return `${year}-${month}-${day}`; }

export function ReportsPage() {
  const today = getLocalDateString(); const monthStart = `${today.slice(0, 8)}01`;
  const [from, setFrom] = useState(monthStart); const [to, setTo] = useState(today); const [report, setReport] = useState<ReportData | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null);
  async function loadReport(start: string, end: string) { setLoading(true); setError(null); try { setReport(await getReport(start, end)); } catch (e: unknown) { setError(e instanceof ApiError ? e.message : "خطا در دریافت گزارش."); } finally { setLoading(false); } }
  useEffect(() => { const timer = window.setTimeout(() => void loadReport(monthStart, today), 0); return () => window.clearTimeout(timer); }, [monthStart, today]);
  return <section className="space-y-6 p-4 md:p-6">
    ...
    <form onSubmit={(e) => { e.preventDefault(); if (from <= to) void loadReport(from, to); else setError("تاریخ شروع باید قبل از تاریخ پایان باشد."); }} className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-3"><label className="text-sm">از تاریخ<JalaliDateInput value={from} onChange={setFrom} className="mt-1 w-full rounded-lg border px-3 py-2" /></label><label className="text-sm">تا تاریخ<JalaliDateInput value={to} onChange={setTo} className="mt-1 w-full rounded-lg border px-3 py-2" /></label><button disabled={loading || !from || !to} className="self-end rounded-lg bg-gray-900 px-4 py-2 text-sm text-white disabled:opacity-50">{loading ? "در حال دریافت..." : "نمایش گزارش"}</button></form>
    ...
    </div><div className="mt-2 text-xl font-bold">{value}</div>{unit ? <div className="mt-1 text-xs text-gray-400">تومان</div> : null}</div>)}</div>
