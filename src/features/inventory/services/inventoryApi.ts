...
export async function getInventory(
  params: { search?: string; product_id?: string; expired?: boolean; sort?: string; page?: number; per_page?: number } = {},
) {
  const response = await apiClient.get<InventoryListResponse>("/inventory-batches", { params });
