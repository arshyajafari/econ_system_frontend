...
export function InventoryPage() {
  ...
  const canManageInventory = isAdmin || isAccountant;
  const [deleteTarget, setDeleteTarget] = useState<string | null>(null),
    [items, setItems] = useState<InventoryBatch[]>([]),
    ...
    [selected, setSelected] = useState<InventoryBatch | null>(null),
    [editing, setEditing] = useState<InventoryBatch | null>(null);
  useEffect(() => {
    let dead = false;
    const run = async () => {
      setLoading(true);
  ...
  const view = async (item: InventoryBatch) => {
    try {
      setError(null);
      setSelected(await getInventoryBatch(item.id));
    } catch (e: unknown) {
      setError(
        e instanceof ApiError ? e.message : "خطا در دریافت جزئیات موجودی.",
      );
    }
  };
  const startEdit = (item: InventoryBatch) => {
    setEditing(item);
  };
  const saveEdit = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      const updated = await updateInventoryBatch(editing.id, {
        batch_number: editing.batch_number ?? "",
        expire_date: editing.expire_date ?? "",
      ...
      setError(e instanceof ApiError ? e.message : "خطا در ویرایش موجودی.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const receivedTime = form.received_at.slice(11, 16);
  return (
    <>
      <ConfirmModal
        open={deleteTarget !== null}
        title="حذف موجودی"
        description="آیا از حذف این بچ موجودی مطمئن هستید؟ این عملیات برگشت‌پذیر نیست."
        confirmLabel="حذف موجودی"
        variant="danger"
      ...
      <section className="space-y-5 p-4 md:p-6">
          ...
          >
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <FormattedNumberInput
                ...
                onValueChange={(value) =>
                  setForm({ ...form, purchase_price: value })
                }
                placeholder="قیمت خرید به ریال"
                className="rounded-lg border px-3 py-2 text-right"
              />
              <JalaliDateInput
                disabled={saving}
                value={form.expire_date}
                onChange={(value) => setForm({ ...form, expire_date: value })}
                className="rounded-lg border px-3 py-2"
                  ...
                  onChange={(event) =>
                    setForm({
                      ...
                      received_at: receivedDate
                        ? `${receivedDate}T${event.target.value}`
                        : "",
                    })
                  }
                  className="rounded-lg border px-3 py-2"
                  aria-label="ساعت ورود"
                />
              </div>
            </div>
        ...
        <div className="w-full flex gap-3">
          ...
          <button
            ...
            className="rounded-lg border px-4 disabled:opacity-50"
          >
            بروزرسانی
          </button>
        </div>
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="px-4 py-3">
                      {item.purchase_price === null
                        ? "—"
                        : money.format(Number(item.purchase_price))}
                    </td>
                    <td className="px-4 py-3">
                      {item.product?.sale_price === null
                        ? "—"
                        : money.format(Number(item.product?.sale_price))}
                    </td>
                    <td className="px-4 py-3">{item.quantity}</td>
                    <td className="px-4 py-3">{item.reserved_quantity}</td>
                    <td className="px-4 py-3 font-medium">
                      {Math.max(0, item.available_quantity)}
                    </td>
                    <td className="px-4 py-3">
                      {formatJalaliDate(item.expire_date)}
                      {item.is_expired && (
                        <span className="mr-2 text-red-600">منقضی</span>
                      )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
              ...
              <div className="grid gap-3 py-5 md:grid-cols-2">
                ...
                <div>
                  <span className="text-xs text-gray-500">کد محصول</span>
                  <p>{selected.product?.code ?? "—"}</p>
                </div>
                <div>
                  <span className="text-xs text-gray-500">شماره بچ</span>
                  <p>{selected.batch_number ?? "—"}</p>
                </div>
                <div>
                ...
                <div>
                  <span className="text-xs text-gray-500">رزرو شده</span>
                  <p>{selected.reserved_quantity}</p>
                </div>
                <div>
                  <span className="text-xs text-gray-500">قابل استفاده</span>
                  <p>{Math.max(0, selected.available_quantity)}</p>
                </div>
                <div>
                  <span className="text-xs text-gray-500">قیمت خرید</span>
                  <p>
                    {selected.purchase_price === null
                      ? "—"
                      : money.format(Number(selected.purchase_price))}
                  </p>
                </div>
                <div>
                  <span className="text-xs text-gray-500">انقضا</span>
                  <p>{formatJalaliDate(selected.expire_date)}</p>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            ...
            >
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold">ویرایش موجودی</h2>
                <button
                  type="button"
              ...
              <div className="text-sm text-gray-500">
                {editing.product?.title ?? "—"} — موجودی فعلی:{" "}
                {editing.quantity}
              </div>
              <div>
                  ...
                  onChange={(e) =>
                    setEditing({
                      ...
                      quantity: Math.max(
                        ...
                        Number(e.target.value) || editing.reserved_quantity,
                      ),
                    })
                  }
                ...
                <p className="mt-1 text-xs text-gray-500">
                  حداقل تعداد مجاز: {editing.reserved_quantity} (تعداد رزروشده)
                </p>
              </div>
              <input
                value={editing.batch_number ?? ""}
                onChange={(e) =>
                  setEditing({ ...editing, batch_number: e.target.value })
                }
                placeholder="شماره بچ"
                className="w-full rounded-lg border px-3 py-2"
              />
              <JalaliDateInput
                value={editing.expire_date ?? ""}
                onChange={(value) =>
                  setEditing({ ...editing, expire_date: value })
                }
                className="w-full rounded-lg border px-3 py-2"
                placeholder="تاریخ انقضا"
              />
