...
export async function getVisitDoctors(): Promise<Doctor[]> { const r = await apiClient.get<{ data: Doctor[] }>("/doctors", { params: { status: "active", per_page: 100 } }); return r.data.data; }
...
export async function deleteVisit(id: string) { await apiClient.delete(`/visits/${id}`); }
