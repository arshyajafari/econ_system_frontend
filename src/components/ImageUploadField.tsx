...
export function ImageUploadField({
  ...
  const preview = objectUrl ?? (file ? null : value ?? null);
  ...
  function selectFile(nextFile: File | null) {
    ...
    if (!nextFile.type.startsWith("image/")) {
      onError?.("فرمت فایل باید تصویر باشد.");
      return;
    }

    if (nextFile.size > maxSizeMB * 1024 * 1024) {
      onError?.(`حجم تصویر نباید بیشتر از ${maxSizeMB} مگابایت باشد.`);
      return;
    }
  ...
  return (
    <div>
      ...
      <div
        ...
        className={`overflow-hidden rounded-2xl border-2 border-dashed bg-gray-50 transition ${
          dragging ? "border-gray-700 bg-gray-100" : "border-gray-300 hover:border-gray-400"
        }`}
      >
        <button
          type="button"
          ...
          className="flex w-full items-center gap-4 p-4 text-right disabled:cursor-not-allowed disabled:opacity-60"
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="font-semibold text-gray-800">
              {file ? "تصویر جدید انتخاب شد" : preview ? "تغییر تصویر" : "انتخاب تصویر"}
            </div>
            ...
            {file ? (
              ...
              <div className="mt-2 text-xs text-gray-500">تصویر فعلی حفظ می‌شود مگر تصویر جدید انتخاب کنید.</div>
            ) : null}
          </div>
        ...
        <div className="border-t border-gray-200 bg-white px-4 py-2 text-xs text-gray-500">
          فایل را اینجا رها کنید یا روی «انتخاب فایل» بزنید.
        </div>
      </div>
    </div>
  );
